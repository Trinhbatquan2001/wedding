...
export const admin = (() => {
  ...
  const getUserStats = () =>
    auth.getDetailUser().then((res) => {
      ...
      document.getElementById("confettiAnimation").checked = Boolean(
        res.data.is_confetti_animation
      );
      ...
      request(HTTP_GET, "/api/stats")
        ...
        .send()
        .then((resp) => {
          document.getElementById("count-comment").textContent = String(
            resp.data.comments
          ).replace(/\B(?=(\d{3})+(?!\d))/g, ".");
          document.getElementById("count-like").textContent = String(
            resp.data.likes
          ).replace(/\B(?=(\d{3})+(?!\d))/g, ".");
          document.getElementById("count-present").textContent = String(
            resp.data.present
          ).replace(/\B(?=(\d{3})+(?!\d))/g, ".");
          document.getElementById("count-absent").textContent = String(
            resp.data.absent
          ).replace(/\B(?=(\d{3})+(?!\d))/g, ".");
        });
  ...
  const tenor = (button) => {
    ...
    request(HTTP_PATCH, "/api/user")
      ...
      .then(() =>
        util
          .notify(`success ${form.value.length ? "add" : "remove"} tenor key`)
          .success()
      )
      .finally(() => {
        form.disabled = false;
  ...
  const openLists = (form, query = null) => {
    ...
    if (form.value && form.value.trim().length > 0) {
      timezones = timezones.filter((tz) =>
        tz.toLowerCase().includes(form.value.trim().toLowerCase())
      );
    }
    ...
    if (query === null) {
      document.addEventListener(
        ...
        (e) => {
          if (
            !form.contains(e.currentTarget) &&
            !dropdown.contains(e.currentTarget)
          ) {
